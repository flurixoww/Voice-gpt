...
########################################################################################
    ...
    # User commands
        ...
        if quest == "open Explorer GPT":
            engine.say("Opening github explorer voice gpt")
            engine.runAndWait()
            os.startfile(r"D:\pytorch\Voice-gpt")
            continue

        if quest == "open speech":
            engine.say("Opening STT directory")
            engine.runAndWait()
            os.startfile(r"D:\Programming\STT")
            continue

        if quest == "open GitHub":
            engine.say("Opening GitHub")
            engine.runAndWait()
            webbrowser.open('https://github.com/flurixoww?tab=repositories', new=2)
            continue
##############################################################################
